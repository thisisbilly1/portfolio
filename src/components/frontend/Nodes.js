    ...
    <context.Provider value={set}>
      <group ref={group}>
        {lines.map((line, index) => (
          <group>
            <QuadraticBezierLine key={`${index}-1`} {...line} color="white" dashed dashScale={50} gapSize={20} />
            <QuadraticBezierLine key={`${index}-2`} {...line} color="white" lineWidth={0.5} transparent opacity={0.1} />
          </group>
        ))}
      </group>
      {children}
      {lines.map(({ start, end }, index) => (
        <group key={`${index}-3`} position-z={1}>
...
export const Node = forwardRef(({ color = 'black', name, connectedTo = [], position = [0, 0, 0], parent, ...props }, ref) => {
  ...
  const bind = useDrag(({ down, xy: [x, y], tap }) => {
    ...
    if (tap){
      ...
      setPos(new THREE.Vector3(((x - left) / size.width) * 2 - 1, -((y - top) / size.height) * 2 + 1, 0).unproject(camera).multiply({ x: 1, y: 1, z: 0 }).clone())
    }
  }, {
    filterTaps: true,
  })
  ...
  return (
    <Circle ref={ref} {...bind()} opacity={0.2} radius={0.5} color={color} position={pos} {...props}>
      <Circle
        ...
        color={hovered || selected ? '#ff1050' : color}>
